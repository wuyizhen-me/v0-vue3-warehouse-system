...
系统提供了SQL脚本用于创建表结构和初始化示例数据：

```bash
# 在MySQL中执行以下脚本
mysql -u root -p warehouse_system < scripts/001_create_schema.sql
mysql -u root -p warehouse_system < scripts/002_seed_data.sql
```

或在v0界面中，直接运行scripts文件夹中的SQL脚本。
