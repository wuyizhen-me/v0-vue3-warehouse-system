...
export async function POST(request: Request) {
  try {
    ...
    const { name, sku, category, description, unit } = body
    ...
    const sql = `
      INSERT INTO products (name, sku, category, description, unit)
      VALUES (?, ?, ?, ?, ?)
    `

    const result: any = await query(sql, [name, sku, category || null, description || null, unit || "件"])
    ...
    if (error.code === "ER_DUP_ENTRY") {
      return NextResponse.json({ success: false, error: "SKU已存在" }, { status: 400 })
    }
